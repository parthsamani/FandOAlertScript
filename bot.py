...
print(f"TOKEN present: {bool(TOKEN)}, CHAT_ID present: {bool(CHAT_ID)}")

for stock in FNO_STOCKS:
    try:
        # FIXED PART - More stable than yf.download on GitHub Actions
        df = yf.Ticker(stock).history(period="5d", interval="5m", auto_adjust=True)

        if df.empty or len(df) < 100:
            print(f"{stock}: No data / less data ({len(df)})")
            time.sleep(0.5)
            continue
